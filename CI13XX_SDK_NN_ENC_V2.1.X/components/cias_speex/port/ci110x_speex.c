...
SpeexMode cias_speex_wb_mode;
//unsigned char g_speex_result[100*1024];

void *ci_speex_encode_create(void)
{
    ...
    speex->_encoder = speex_encoder_init(&cias_speex_wb_mode); /*16K采样率*/  //speex_nb_mode-20ms一帧 160个采样点 cias_speex_wb_mode-320个采样点
    ...
    speex->_buffer_out = (char *)pvPortMalloc(speex->_frame_size*2);
    ...
    speex->_buffer_in = (short *)pvPortMalloc(sizeof(short) * (speex->_frame_size));   //malloc大小必须和实际数据大小符合，不然会出现解码后开头数据错误
    ...
    speex->_buffer_in_float = (float *)pvPortMalloc(sizeof(float) * (speex->_frame_size));   //malloc大小必须和实际数据大小符合，不然会出现解码后开头数据错误
    ...
    mprintf("ci_speex_encode_create,Speex frame size: %d\r\n", speex->_frame_size);
...
void *ci_speex_decode_create(void)
{
    ...
    speex->_decoder = speex_decoder_init(&cias_speex_wb_mode);   /*16K采样率*/
    if (speex->_decoder == NULL)
    {
        mprintf("Speex encoder state init failed!\n");
    }
    ...
    speex_decoder_ctl(speex->_decoder, SPEEX_SET_QUALITY, &quality);
    // speex_encoder_ctl(speex->_encoder, SPEEX_SET_COMPLEXITY, &complexity);
    speex_decoder_ctl(speex->_decoder, SPEEX_GET_FRAME_SIZE, &speex->_frame_size);

    speex->_buffer_out = (char *)pvPortMalloc(speex->_frame_size);
    if (speex->_buffer_out == NULL)
    {
        mprintf("Memory overflow!!! Alloc speex buffer out failed.\n");
    }

    speex->_buffer_in = (short *)pvPortMalloc(sizeof(short) * speex->_frame_size);
    if (speex->_buffer_in == NULL)
    {
        mprintf("Memory overflow!!! Alloc speex buffer in failed.\n");
    }

    speex->_buffer_in_float = (float *)pvPortMalloc(sizeof(float) * speex->_frame_size);
    if (speex->_buffer_in_float == NULL)
    {
        mprintf("Memory overflow!!! Alloc speex buffer in failed.\n");
    }

    mprintf("cias_speex_create,Speex frame size: %d\n", speex->_frame_size);
    return speex;
...
void speex_test(unsigned int pcm_addr, unsigned int raw_addr, unsigned int pcm_size)
{
    ...
    ci_speex_hander = ci_speex_encode_create();
...
// 	enc_state = speex_encoder_init(&cias_speex_wb_mode);
