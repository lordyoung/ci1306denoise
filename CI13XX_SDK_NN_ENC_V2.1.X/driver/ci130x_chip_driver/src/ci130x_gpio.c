...
void GPIO_IRQ_Default_proc(gpio_base_t base, int gpio_port_index)
{
	...
	for (int i = 0; i < 8; i++)
	{
		...
		//	ci_loginfo(LOG_GPIO_DRIVER,"GPIO%d Pin%d IRQ\n", gpio_port_index, i);
            gpio_status = gpio_get_input_level_single(base, i); // 获取IO状态
            _delay_10us_240M(10000); // 10ms去抖
            if(gpio_status == gpio_get_input_level_single(base, i));
            {   extern void rtc_key_handle(gpio_base_t base, int gpio_port_index);
                rtc_key_handle(base, i);
            }
		}
	}
}
