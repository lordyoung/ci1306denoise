...
void UARTDMAConfig(UART_TypeDef *UARTx, UART_BaudRate uartbaudrate)
{
    ...
    UART_FIFOClear(UARTx);
    UART_CRConfig(UARTx, UART_NCED, ENABLE);
    //UART_TXRXDMAConfig(UARTx, UART_RXDMA);       //屏蔽RX的DMA使能
    UART_TXRXDMAConfig(UARTx, UART_TXDMA);

    UART_IntMaskConfig(UARTx, UART_TXInt, ENABLE); //关闭TX中断
    UART_IntMaskConfig(UARTx, UART_RXInt, DISABLE); //打开RX中断
    ...
    eclic_irq_enable(Uart_IRQ_Num);  //使能中断
}
