...
//通道数据选择
short *iis_audio_choose(audio_channel_model channel_model, ci_wrapfft_audio *wrapfft_audio_t, short *iis_dst_data)
{
	short *dst_data;
...
uint8_t g_start_handle_ifft_data_flag = 0;
extern QueueHandle_t ifft_data_queue;
...
*/
int ifft_data_to_eq_drc_handle(uint32_t eq_config_addr, uint32_t drc_config_addr, ci_wrapfft_audio *wrapfft_audio_st, short *dst_data, uint32_t iis_out_audio_addr)
{
    int ret = 0;
    g_eq_param = (npu_eq_param_t *)eq_config_addr;
    g_drc_param = (npu_drc_param_t *)drc_config_addr;
    g_start_handle_ifft_data_flag = 1;
    //EQ和DRC处理
#if USE_EQ_MODULE
	...
	if (iis_out_audio->iis_out_enable)
	{
		...
		audio_pre_rslt_write_data(left_data, right_data);
	}
