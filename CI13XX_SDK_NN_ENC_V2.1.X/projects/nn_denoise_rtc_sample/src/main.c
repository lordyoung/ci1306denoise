...
static void welcome(void)
{
    ...
    ci_loginfo(LOG_USER, "Check ipcore clk\r\n");
    // 实际主频检查
...
static void task_init(void *p_arg)
{
    ...
    xTaskCreate(audio_in_manage_inner_task_rtc,"audio_in_manage_inner_task_rtc", 300, NULL, 4, NULL);
    #if USE_IIS0_AUDIO_IN_SPEAKER_OUT
    extern void iis0_speaker_out_task(void *p);
    xTaskCreate(iis0_speaker_out_task,"iis0_speaker_out_task", 300, NULL, 4, NULL);
    #endif
    #if USE_UART_SEND_PRE_RSLT_AUDIO
    ...
    uart_eq_param_cfg_init();

    userapp_initial();  //初始化协议串口和key
